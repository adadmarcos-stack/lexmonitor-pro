...
def is_ignorable_line(line: str) -> bool:
    ...
    boilerplates = [
        ...
        "visualizar tudo",
        "voltar para seleção",
        "resumo por data de disponibilização",
    ]
...
async def click_if_exists(page, selectors: List[str], label: str, wait_ms: int = 4000) -> bool:
...
async def try_fill_date_filters(page, days_back: int = 60):
    ...
    print(f"Preenchendo datas: {start} até {end}")
    ...
    if start_filled:
        ...
        print("Data inicial não localizada.")

    if end_filled:
        print("Data final preenchida.")
    else:
        print("Data final não localizada.")

    clicked = await click_if_exists(page, possible_submit, "CONSULTAR", wait_ms=5000)
    if not clicked:
        print("⚠️ Botão Consultar não encontrado após preencher datas.")


async def wait_for_oab_result(page, days_back: int = 60):
    ...
    await save_debug_screenshot(page, "historico_after_visualizar_tudo")
    print("Tela de publicações carregada ou tentativa concluída.")
...
async def scrape_oab(days_back: int = 60) -> List[Dict]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("⏳ Indo para fluxo pós-login...")
            await wait_for_oab_result(page, days_back=days_back)

            print("📄 Capturando conteúdo da página...")
...
def run_monitor(days_back: int = 60) -> Dict:
    ...
    try:
        ...
        print(f"JusBrasil indisponível: {e}")
